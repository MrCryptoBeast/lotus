...
var autoTaskCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		mInfo, err := nodeApi.GetStateMinerInfo(ctx, maddr)
		if err != nil {
			nodeApiCloser()
			return err
		}
		...
		for {
			...
			case <-timer.C:
				...
				workerBalance, err := mapi.GetWalletBalance(lcli.ReqContext(cctx), mInfo.Worker)
				if err != nil {
					log.Errorf("auto task connect Miner Api failed: %w", err)
					minerApiCloser()
					continue
				}
