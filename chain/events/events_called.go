...
func (e *hcEventsObserver) applyTimeouts(ctx context.Context, at abi.ChainEpoch, head *types.TipSet) {
	...
	for triggerID, calls := range triggers {
		...
		timeoutTs, err := e.cs.ChainGetTipSetAfterHeight(ctx, at-abi.ChainEpoch(trigger.confidence), head.Key())
		if err != nil {
			log.Errorf("events: applyTimeouts didn't find tipset for event; wanted %d; current %d", at-abi.ChainEpoch(trigger.confidence), at)
		}

		more, err := trigger.handle(nil, nil, nil, timeoutTs, at)
		if err != nil {
