...
func (b *PreCommitBatcher) maybeStartBatch(notif bool) ([]sealiface.PreCommitBatchRes, error) {
	...
	if !cfg.BatchPreCommitAboveBaseFee.Equals(big.Zero()) {
		...
		if bf.LessThan(cfg.BatchPreCommitAboveBaseFee) && nv >= network.Version14 {
			individual = true
		}
	}
...
func (b *PreCommitBatcher) processBatch(cfg sealiface.Config) ([]sealiface.PreCommitBatchRes, error) {
	...
	bf, err := b.api.ChainBaseFee(b.mctx, tok)
	if err != nil {
		return []sealiface.PreCommitBatchRes{res}, xerrors.Errorf("couldn't get base fee: %w", err)
	}

	nv, err := b.api.StateNetworkVersion(b.mctx, tok)
	if err != nil {
		log.Errorf("getting network version: %s", err)
		return []sealiface.PreCommitBatchRes{res}, xerrors.Errorf("getting network version: %s", err)
	}
